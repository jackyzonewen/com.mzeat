...
public class BuyActivity extends BaseActivity {
					...
					if (data_near.size() == 0) {
						...
						if (!MzeatApplication.getInstance().lat.equals("")) {
							...
							if (CheckNetworkConnection
									.checkNetworkConnection(BuyActivity.this)) {

								if (null != mLoadDataTask
										&& mLoadDataTask.getStatus() == GenericTask.Status.RUNNING) {
									ShowToast.showToastShort(BuyActivity.this,
			...
			if (result == TaskResult.OK) {
				...
				if (page == 1) {
					...
					case check_normal:
						...
						if (!save_local) {
							mBuyDb = new BuyDb(BuyActivity.this);
							mBuyDb.add(data_normal);
							mBuyDb.closeDB();
						}
						break;
					default:
				...
				mLoadDataTask = null;
				ShowToast.showGetFaile(BuyActivity.this);

			} else if (result == TaskResult.IO_ERROR) {
				mLoadDataTask = null;
				ShowToast.showError(BuyActivity.this);
			} else if (result == TaskResult.NO_MORE_DATA) {
				mLoadDataTask = null;
				ShowToast.showNomoredata(BuyActivity.this);
			}
	...
	private void getOldFood() {
		...
		mBuyDb = new BuyDb(this);
		data_normal = mBuyDb.getShoppings();
		mBuyDb.closeDB();
		if (data_normal.size() != 0) {
			mAdapter.setDataList(data_normal);
			mListView.setAdapter(mAdapter);
