...
public class HappyActivity extends BaseActivity {
	...
	private HappyDb mHappyDb;
	public final static String SER_KEY = "happy";
	...
	//	pager = new CustomViewpage(this);
		...
		btn_back.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				// Log.e("TEST","FINISH");
				Intent intent = new Intent(HappyActivity.this,
						MainActivity.class);
					...
					if (data_near.size() == 0) {
						...
						if (!MzeatApplication.getInstance().lat.equals("")) {
							...
							if (CheckNetworkConnection
									.checkNetworkConnection(HappyActivity.this)) {

								if (null != mLoadDataTask
										&& mLoadDataTask.getStatus() == GenericTask.Status.RUNNING) {
									ShowToast.showToastShort(HappyActivity.this,
			...
			if (result == TaskResult.OK) {
				...
				if (page == 1) {
					...
					case check_normal:
						...
						if (!save_local) {
							 mHappyDb = new  HappyDb(HappyActivity.this);
							 mHappyDb.add(data_normal);
							 mHappyDb.closeDB();
						}
						break;
					default:
				...
				mLoadDataTask = null;
				ShowToast.showGetFaile(HappyActivity.this);

			} else if (result == TaskResult.IO_ERROR) {
				mLoadDataTask = null;
				ShowToast.showError(HappyActivity.this);
			} else if (result == TaskResult.NO_MORE_DATA) {
				mLoadDataTask = null;
				ShowToast.showNomoredata(HappyActivity.this);
			}
	...
	private void getOldFood() {
		// mShoppings = new ArrayList<Shopping>();
		 mHappyDb = new  HappyDb(this);
		 data_normal =  mHappyDb.getShoppings();
		 mHappyDb.closeDB();
		if (data_normal.size() != 0) {
			mAdapter.setDataList(data_normal);
			mListView.setAdapter(mAdapter);
