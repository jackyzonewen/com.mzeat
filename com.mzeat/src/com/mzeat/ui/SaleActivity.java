...
import com.mzeat.ui.adapter.SaleListAdapter;
...
public class SaleActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mListView = (PullToRefreshListView) findViewById(R.id.sale_list);
		mListView.setOnRefreshListener(RefreshListener);
		mAdapter = new SaleListAdapter(this);
	...
	ArrayList<Sale> mSales = new ArrayList<Sale>();
		...
		public void onPostExecute(GenericTask task, TaskResult result) {
			...
			if (result == TaskResult.OK) {
				...
				if (page == 1) {
					...
					mSales = mLoadDataTask.getSale();
					
					mAdapter.setDataList(mSales);
					mAdapter.notifyDataSetChanged();
					mListView.setAdapter(mAdapter);
					load_result = load_fromnetwork_success;

					if (!save_local) {
						mDb = new SaleDb(SaleActivity.this);
						mDb.add(mSales);
						mDb.closeDB();
					}
					...
					mSales.addAll(mLoadDataTask.getSale());
					mAdapter.notifyDataSetChanged();
					mLoadDataTask = null;
				}
				...
				mLoadDataTask = null;
				ShowToast.showGetFaile(SaleActivity.this);

			} else if (result == TaskResult.IO_ERROR) {
				mLoadDataTask = null;
				ShowToast.showError(SaleActivity.this);
			} else if (result == TaskResult.NO_MORE_DATA) {
				mLoadDataTask = null;
				ShowToast.showNomoredata(SaleActivity.this);
			} else if (result == TaskResult.NOT_FOLLOWED_ERROR) {
				ShowToast.showNoOrder(SaleActivity.this);
			}
	...
	private SaleReturn result = new SaleReturn();
	...
	private class LoadDataTask extends GenericTask {
		...
		protected TaskResult _doInBackground(TaskParams... params) {
			...
			if (mPage == null) {
				result = MzeatApplication.getInstance().getService()
						.getSaleReturn(String.valueOf(page));
				int code = Integer.valueOf(result.getOpen());
				if (code == MzeatService.RESULT_OK) {
					sales = (ArrayList<Sale>) result.getItem();
					Log.e("sales", String.valueOf(sales.size()));
		...
		private ArrayList<Sale> getSale() {
			return sales;
		}
	}
	...
	private PullToRefreshBase.OnRefreshListener<ListView> RefreshListener = new PullToRefreshBase.OnRefreshListener<ListView>() {
		...
		public void onRefresh(PullToRefreshBase<ListView> refreshView) {
			...
			if (CheckNetworkConnection
					.checkNetworkConnection(SaleActivity.this)) {
				loaddata();

			} else {
				ShowToast.showToastShort(SaleActivity.this,
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			// TODO Auto-generated method stub
					Intent intent = new Intent(SaleActivity.this,
							PrivilegeWeb.class);
					String content  = mSales.get(position - 1).getContent();
					intent.putExtra("context", content);
					intent.putExtra("title",  mSales.get(position - 1).getTitle());
					intent.putExtra("create_time",  mSales.get(position - 1).getCreate_time());
					intent.putExtra("from", FROM_SALE);
