...
public class ImageZoomDialog extends BaseActivity implements OnTouchListener, OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 0; i <url_imgs.size()  ; i++) {
			...
			mViewSwitcher  = (ViewSwitcher) LayoutInflater.from(
					ImageZoomDialog.this).inflate(R.layout.image_zoom_dialog,
					null);
			//progressBar = (ProgressBar) mViewSwitcher.findViewById(R.id.imagezoomdialog_progress);
				
			imgView =  (ImageView) mViewSwitcher.findViewById(R.id.imagezoomdialog_image);
			imgView.setOnTouchListener(new  MulitPointTouchListener());// 设置触屏监听
			final String imgURL = url_imgs.get(i);
				...
				public void run() {
					...
					try {
						...
						if(imgURL.endsWith("portrait.gif") || StringUtils.isEmpty(imgURL)){
							bmp = BitmapFactory.decodeResource(imgView.getResources(), R.drawable.empty_image);
						}
						if(bmp == null){
							//是否有缓存图片
