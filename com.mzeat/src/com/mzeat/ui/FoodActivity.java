...
public class FoodActivity extends BaseActivity {
	...
	public final static String SER_KEY = "food";
					...
					if (data_near.size() == 0) {
						...
						if (!MzeatApplication.getInstance().lat.equals("")) {
							...
							if (CheckNetworkConnection
									.checkNetworkConnection(FoodActivity.this)) {

								if (null != mLoadDataTask
										&& mLoadDataTask.getStatus() == GenericTask.Status.RUNNING) {
									ShowToast.showToastShort(FoodActivity.this,
			...
			if (result == TaskResult.OK) {
				...
				if (page == 1) {
					...
					case check_normal:
						...
						if (!save_local) {
							mFoodDb = new FoodDb(FoodActivity.this);
							mFoodDb.add(data_normal);
							mFoodDb.closeDB();
						}
						break;
					default:
				...
				mLoadDataTask = null;
				ShowToast.showGetFaile(FoodActivity.this);

			} else if (result == TaskResult.IO_ERROR) {
				mLoadDataTask = null;
				ShowToast.showError(FoodActivity.this);
			} else if (result == TaskResult.NO_MORE_DATA) {
				mLoadDataTask = null;
				ShowToast.showNomoredata(FoodActivity.this);
			}
	...
	private void getOldFood() {
		...
		mFoodDb = new FoodDb(this);
		data_normal = mFoodDb.getShoppings();
		mFoodDb.closeDB();
		if (data_normal.size() != 0) {
			mAdapter.setDataList(data_normal);
			mListView.setAdapter(mAdapter);
	...
	private static final int fromfood = 1;
