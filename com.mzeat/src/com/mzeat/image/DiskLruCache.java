 ...
 * for bitmap caching. 
 * 一个简单的内存LRU位图缓存来举例如何为位图缓冲使用内存缓存
 * A much more robust and efficient disk LRU cache solution can be found in the ICS source code
...
public class DiskLruCache {
	 ...
	 * 不要直接调用构造方法，在建立DiskLruCache实例之前要运行几个额外的检查。
	 * @param cacheDir
	 * @param maxByteSize
	 */
	...
	public static File getDiskCacheDir(Context context, String uniqueName) {
		...
		// 检查是否有安装好的多媒体和插入的存储卡，如果有，尝试使用外部的缓存目录，否则使用内部的缓存目录。
